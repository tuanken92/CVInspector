...
CString CXmlNodeWrapper::GetValue(LPCTSTR valueName)
{
	...
	if (attribute)
	{
		return CString((TCHAR*)attribute->Gettext());
	}
	return _T("");
}

int CXmlNodeWrapper::GetValueInt(LPCTSTR valueName, int iDefault)
{
	if (!IsValid())
		return iDefault;
	...
	if (attribute)
	{
		int iRet = _ttoi((TCHAR*)attribute->Gettext());		
		return iRet;
	}
...
double CXmlNodeWrapper::GetValueDouble(LPCTSTR valueName, double dDefault)
{
	if (!IsValid())
		return dDefault;
	...
	if (attribute)
	{
		double dRet = atof((const char *)attribute->Gettext());
		return dRet;
	}
	return dDefault;
}

BOOL CXmlNodeWrapper::GetValueBool(LPCTSTR valueName, BOOL bDefault)
{
	if (!IsValid())
		return bDefault;
	...
	if (attribute)
	{
		CString strText = (TCHAR*)attribute->Gettext();
		if(strText.Compare(_T("True")) == 0 || strText.Compare(_T("1")) == 0)
		{
			return TRUE;
		}		
		return FALSE;
...
{
	if (IsValid())
		return CString((TCHAR*)m_xmlnode->Getxml());
	else
		return _T("");
}
...
{
	if (IsValid())
		return CString((TCHAR*)m_xmldoc->Getxml());
	else
		return _T("");
}
...
{
	return CString((TCHAR*)m_xmldoc->Geturl());
}
...
{
	if (IsValid())
		return CString((TCHAR*)m_xmlnode->GetbaseName());
	return _T("");
}
...
{
	if (IsValid())
		return CString((TCHAR*)m_xmlnode->Gettext());
	else
		return _T("");
}
...
CString CXmlNodeWrapper::GetAttribName(int index)
{
	if (IsValid())
	{
		...
		if (attribs)
		{
			...
			if (attrib)
				return CString((TCHAR*)attrib->Getname());
		}
	}
...
CString CXmlNodeWrapper::GetAttribVal(int index)
{
	if (IsValid())
	{
		...
		if (attribs)
		{
			MSXML2::IXMLDOMAttributePtr attrib = attribs->Getitem(index);
			if (attrib)
				return CString((TCHAR*)attrib->Gettext());
		}
	}
...
{
	if (IsValid())
		return CString((TCHAR*)m_xmlnode->GetnodeTypeString());
	return _T("");
}
